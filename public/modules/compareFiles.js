...
// export const compareFiles = (csvFile, txtFile) => {
//     const resultObject = [];

//     // Loop throuh each csv and txt file
//     loopBothArrays(csvFile, txtFile, (csvArray, txtArray) => {
//         const sameRowsByIdNumber = [];
//         const wrongVatNumberInvoices = [];
//         const wrongVatValueInvoices = [];

//         // Loop throuh each csv and txt array
//         loopBothArrays(csvArray, txtArray, (csvRow, txtRow) => {
//             getInvoicesWithSameId(csvRow, txtRow, sameRowsByIdNumber, wrongVatNumberInvoices, wrongVatValueInvoices);
//         })

//         // Extract those invoices that are missing 
//         const [missingInvoices] = getMissingInvoices(csvArray, sameRowsByIdNumber);

//         // Extract only the Title from the csv file
//         const [missingInvoicesTitle] = getTitleFromCsvFile(missingInvoices);

//         // Extract only the Header from the csv file
//         const [missingInvoicesHeader] = getHeaderFromCsvFile(missingInvoices);

//         // Extract only the invoices without any headers and titles 
//         const [missingInvoicesWithoutTitleAndHeader] = removeTitleAndHeaderInMissingInvoices(missingInvoices);

//         // Create an object that contains all the result after comparison as well as category for each, title and header where available. 
//         const [result] = extractResults({
//             missingInvoicesTitle, 
//             missingInvoicesHeader,
//             missingInvoicesWithoutTitleAndHeader,
//             wrongVatNumberInvoices,
//             wrongVatValueInvoices 
//         })
...
// export const compareFiles = (csv, txt) => {
//     const sameRowsByIdNumber = [];
//     const resultObject = [];

//     csv.forEach(csvArray => {
//         const wrongVatNumberInvoices = [];
//         const wrongVatValueInvoices = [];

//         txt.forEach(txtArray => {
//             csvArray.forEach(csvRow => {
//                 txtArray.forEach(txtRow => {
//                     // Check for those with the same id's and those starting only with "BG" and extract them
//                     if (csvRow[csvFileIdCell-1] === txtRow[txtFileIdCell-1] && csvRow[0].startsWith("BG")) {
//                         sameRowsByIdNumber.push(csvRow);
...
//                         // Check for those that have a wrong VAT amount and extract them
//                         if (csvRow[csvFileVatValueCell-1] !== txtRow[txtFileVatValueCell-1]) {
//                             wrongVatValueInvoices.push(txtRow);
//                         }
//                     }
//                 })
//             })
//         })
//         // Extract those invoices that are missing 
//         // const missingInvoices = csvArray.filter(row => !sameRowsByIdNumber.includes(row));
//         const [missingInvoices] = getMissingInvoices(csvArray, sameRowsByIdNumber);
...
//     csvArray.forEach(csvRow => {
//         txtArray.forEach(txtRow => {
//             if (csvRow[csvFileIdCell-1] === txtRow[txtFileIdCell-1]) {
//                 sameRowsByIdNumber.push(csvRow);
//             }
//         })
//     })
//     return [csvArray.filter(row => !sameRowsByIdNumber.includes(row))];
// }

// const getTheCommonRows = () => {
//     const sameRowsByIdNumber = [];
...
// }
    ...
    // csvArray.forEach(csvRow => {
    //     txtArray.forEach(txtRow => {
    //         // Extract those with the same id's
    //         if (csvRow[csvFileIdCell-1] === txtRow[txtFileIdCell-1]) {
    //             sameRowsByIdNumber.push(csvRow);

    //             // Check if inside those with same id's there is some that have a wrong VAT number and extract them 
    //             // if (csvRow[0] !== txtRow[4]) {
    //             //     sameRowsByVatNumber.push(txtRow);
    //             // }

    //             // Check for those that have the same VAT amount and extract them
    //             if (csvRow[8] === txtRow[10]) {
    //                 sameRowsByVatAmaount.push(txtRow);
    //             }
...
// sameRowsByIdNumber.forEach(sameRow => {
    //     txtArray.forEach(txtRow => {
    //         // Check if inside those with same id's there is some that have a wrong VAT number and extract them. 
    //         if (sameRow[0] === txtRow[4]) {
    //             sameRowsByVatNumber.push(txtRow);
    //         }
    //     })
    // })
    ...
    // // Extract those invoices that are missing 
    // const wrongVatNumberInvoices = txtArray.filter(row => !sameRowsByVatNumber.includes(row));